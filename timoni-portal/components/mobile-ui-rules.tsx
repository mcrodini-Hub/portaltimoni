...
function correctGuidePermissions(root:ParentNode){if(!window.location.pathname.includes("/configuracoes"))return;root.querySelectorAll<HTMLElement>("summary span").forEach(el=>{if(normalizeText(el.textContent||"")==="7. configuracoes — cica e marcelo")el.textContent="7. Configurações — permissões"});root.querySelectorAll<HTMLElement>("p").forEach(el=>{const text=normalizeText(el.textContent||"");if(text.includes("cica e marcelo podem administrar acessos, permissoes, boxes do painel, colaboradores, backups e historico")){el.textContent="Somente Ciça tem autorização total no Portal. Marcelo mantém apenas as permissões de gestão já definidas para o seu acesso, sem autorização total. Ao alterar permissões, revise com cuidado quais módulos e boxes ficarão disponíveis para cada usuário."}})}
...
function applyRules(){const root=document.body;if(!root)return;hideExplanatoryText(root);correctGuidePermissions(root);forceGeneralView(root);configureStockCards(root);configureMeetingCards(root)}
