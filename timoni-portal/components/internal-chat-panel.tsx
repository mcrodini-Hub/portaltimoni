                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-2">
                      <span className={`block min-w-0 flex-1 whitespace-normal break-words text-[13px] leading-4 sm:text-[10px] ${hasUnread ? "font-bold text-slate-950" : "font-semibold text-slate-800"}`}>{item.name}</span>
                    ...
                    <span className={`mt-0.5 block truncate text-[12px] sm:mt-1 sm:text-[10px] ${hasUnread ? "font-semibold text-slate-800" : "text-slate-600"}`}>
        ...
        <div className={`${selected ? "flex" : "hidden sm:flex"} min-w-0 flex-1 flex-col`}>
          <div className="flex min-h-10 shrink-0 items-center gap-1.5 border-b border-slate-200 bg-white px-2.5 sm:min-h-12 sm:gap-3 sm:px-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-semibold text-slate-950 sm:text-[10px]">{contact?.name || "Selecione uma conversa"}</p>
                ...
                {messages.map((message) => {
                  ...
                  return (
                    <div key={message.id} className={`flex items-center gap-1.5 ${mine ? "justify-end" : "justify-start"}`}>
                      <div className={`w-fit max-w-[78%] rounded-2xl border px-2.5 py-1.5 text-[13px] font-medium leading-[1.05rem] shadow-sm sm:max-w-[64%] sm:px-3 sm:text-[10px] sm:font-normal sm:leading-[1.05rem] ${mine ? "border-[#2296E8]/20 bg-[#F1F8FE] text-[#0F2D8F]" : "border-slate-300 bg-white text-slate-900"}`}>
          ...
          <div data-chat-footer className="shrink-0 border-t border-slate-200 bg-white p-2 pb-[max(0.45rem,env(safe-area-inset-bottom))] sm:p-3">
            ...
            <div data-chat-composer className="flex min-w-0 items-end gap-1.5 overflow-hidden rounded-xl border border-[#2296E8]/30 bg-white p-1.5 shadow-sm focus-within:border-[#2296E8] sm:gap-2 sm:rounded-2xl sm:p-2">
              <textarea value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void sendMessage(); } }} rows={1} maxLength={4000} placeholder={contact ? `Mensagem para ${contact.name}` : "Selecione uma conversa"} className="max-h-24 min-h-9 min-w-0 flex-1 resize-none bg-white px-2 py-2 text-[12px] font-normal leading-4 text-[#0F2D8F] caret-[#0F2D8F] outline-none placeholder:text-[#0F2D8F]/45 sm:max-h-28 sm:min-h-10 sm:text-[10px] sm:leading-4" />
