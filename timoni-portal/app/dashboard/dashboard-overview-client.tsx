...
export default function DashboardOverviewClient({ modules, readOnly }: DashboardOverviewProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [comprasResult, notificationsResult, eventsResult] = await Promise.allSettled([
  ...
  const moduleStatus = (module: string) => {
    if (module === "compras") {
      return {
        value: metric(snapshot.comprasPendentes),
        label: snapshot.comprasUrgentes ? `${snapshot.comprasUrgentes} urgente${snapshot.comprasUrgentes === 1 ? "" : "s"}` : "pedidos pendentes",
        attention: (snapshot.comprasUrgentes ?? 0) > 0,
      };
    }
    if (module === "estoque") {
      return {
        value: metric(snapshot.estoquePendentes),
        label: "solicitações pendentes",
        attention: (snapshot.estoquePendentes ?? 0) > 0,
      };
    }
    if (module === "agenda") {
      return {
        value: metric(snapshot.agendaProximos),
    ...
    if (module === "painel") {
      return {
        value: metric(snapshot.novidades),
        label: "novidades para consultar",
        attention: (snapshot.novidades ?? 0) > 0,
      };
    }
    return null;
  };
    ...
    <div className="space-y-5">
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Link href="/dashboard/estoque" className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
          ...
          <p className="mt-2 text-3xl font-bold tracking-tight text-slate-950">{metric(snapshot.estoquePendentes)}</p>
          <p className="mt-1 text-xs text-slate-600">solicitações pendentes</p>
        </Link>
        <Link href="/agenda" className="rounded-2xl border border-violet-200 bg-violet-50 p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
          <p className="text-xs font-semibold uppercase tracking-wide text-violet-700">Agenda</p>
            ...
            return (
              <Link key={item.name} href={item.href} className={`group rounded-2xl border p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${item.accent}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  {status ? (
                    <span className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${status.attention ? "bg-red-100 text-red-700" : "bg-white text-slate-700"}`}>{status.value}</span>
                  ) : (
                    <span className="rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-slate-600">{readOnly ? "Leitura" : "Acessar"}</span>
                  )}
                </div>
