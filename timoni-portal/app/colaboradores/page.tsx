...
import { AVISO_TEAM_MEMBERS } from "@/lib/team-members";
...
function AnnouncementCard({store,isAdmin,canConfirmRead}:{store:Store;isAdmin:boolean;canConfirmRead:boolean}){const unit=store==="rio claro"?"Rio Claro":"Araras";const members=AVISO_TEAM_MEMBERS.filter(member=>member.unit===unit);return <article className="rounded-3xl border border-blue-200 bg-blue-50 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-blue-700">AVISOS</p><h2 className="mt-2 text-xl font-semibold text-slate-950">{unit}</h2><div className="mt-4 space-y-4"><ComunicadosFeed store={store} isAdmin={isAdmin} canConfirmRead={canConfirmRead} members={members}/></div></article>}
function MeetingSummaryCard({title,list}:{title:string;list:FixedMeeting[]}){return <article className="rounded-3xl border border-violet-200 bg-violet-50 p-5 shadow-sm"><p className="text-xs font-semibold uppercase tracking-wider text-violet-700">Reuniões</p><h3 className="mt-3 font-semibold text-slate-950">{title}</h3>{list.length?<div className="mt-2 space-y-2">{list.map((m,i)=><div key={m.start} className="rounded-xl bg-white/70 p-3"><p className="text-xs font-semibold uppercase text-violet-700">{i===0?"Próxima":"Seguinte"}</p><p className="mt-1 text-sm font-semibold text-slate-800">{formatMeeting(m)}</p></div>)}</div>:<p className="mt-2 text-sm text-slate-500">Nenhuma reunião programada.</p>}</article>}
...
export default async function ColaboradoresPage(){
 const session=await auth();const email=session?.user?.email??"";const normalized=email.trim().toLowerCase();const panelStore=getPanelStore(email);const isCica=normalized==="mcrodini@gmail.com";const canConfirmRead=!["mcrodini@gmail.com","mrodini@gmail.com"].includes(normalized);const now=new Date();let events:CalendarEventDTO[]=[];
 ...
 {panelStore==="geral"?<section className="grid items-start gap-4 lg:grid-cols-2"><AnnouncementCard store="rio claro" isAdmin={isCica} canConfirmRead={canConfirmRead}/><AnnouncementCard store="araras" isAdmin={isCica} canConfirmRead={canConfirmRead}/><MeetingSummaryCard title="Rio Claro" list={rioClaro}/><MeetingSummaryCard title="Araras" list={araras}/><EventCard title="Aniversários" tone="pink" events={birthdays}/><EventCard title="Férias" tone="amber" events={vacations}/></section>:<section className="grid items-start gap-4 lg:grid-cols-2"><AnnouncementCard store={panelStore} isAdmin={isCica} canConfirmRead={canConfirmRead}/><MeetingSummaryCard title={panelStore==="araras"?"Araras":"Rio Claro"} list={panelStore==="araras"?araras:rioClaro}/><EventCard title="Aniversários" tone="pink" events={birthdays}/><EventCard title="Férias" tone="amber" events={vacations}/></section>}
