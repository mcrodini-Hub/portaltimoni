...
export default function MobilePortalHeader({ items, showUpdates, showGuide, showChat, initials }: { items: MobileNavItem[]; showUpdates: boolean; showGuide: boolean; showChat: boolean; initials: string }) {
  ...
  return <>
    <div className="portal-mobile-header sticky top-0 z-[90] grid h-16 grid-cols-[2.75rem_1fr_auto] items-center bg-[#0F2D8F] px-3 text-white sm:hidden">
      ...
      <div className="flex items-center justify-end gap-0.5">
        {showChat ? <ChatButton mobileHeader /> : null}
    ...
    {open ? createPortal(<div id="portal-mobile-menu" className="fixed inset-0 z-[100] sm:hidden" role="dialog" aria-modal="true" aria-label="Menu principal">
      ...
      <aside className="absolute inset-y-0 left-0 flex w-[72%] max-w-[16rem] flex-col bg-white shadow-2xl">
        <div className="flex h-12 items-center justify-between border-b border-slate-100 px-3.5">
          <Link href="/dashboard" onClick={() => setOpen(false)} className="flex items-center text-[#0F2D8F]" aria-label="Casa Timoni — Painel">
            <span className="text-sm font-bold tracking-tight">Casa Timoni</span>
          </Link>
        ...
        <nav className="flex-1 overflow-y-auto px-2.5 py-1.5" aria-label="Menu mobile">
          {items.map((item, index) => {
            const pending = item.updateModule ? updates[item.updateModule] : undefined;
            const active = (item.targetHref ?? item.href) === "/dashboard";
            return <Link key={`${item.label}-${index}`} href={item.targetHref ?? item.href} target={item.external ? "_blank" : undefined} rel={item.external ? "noopener noreferrer" : undefined} onClick={() => void openItem(item)} className={`flex min-h-8 items-center rounded-md px-2 text-[13px] leading-4 text-slate-800 transition ${active ? "bg-blue-50 font-bold text-[#0F2D8F]" : "font-normal hover:bg-slate-50"}`}>
          ...
          {showGuide ? <Link href="/configuracoes#guia" onClick={() => setOpen(false)} className="mt-1 flex min-h-8 items-center border-t border-slate-200 px-2 pt-1 text-[13px] font-normal text-slate-800"><span className="flex-1">Guia de uso</span></Link> : null}
          <button type="button" onClick={() => void signOut({ callbackUrl: "/login" })} className="mt-1 flex min-h-8 w-full items-center border-t border-slate-200 px-2 pt-1 text-left text-[13px] font-normal text-slate-800"><span>Sair</span></button>
        </nav>
      </aside>
