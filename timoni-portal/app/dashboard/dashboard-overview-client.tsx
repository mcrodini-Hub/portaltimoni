...
export default function DashboardOverviewClient({modules,motoristaControle,espacoEquipeControle}:Props){
 ...
 useEffect(()=>{let off=false; async function load(){const start=new Date();start.setHours(0,0,0,0);const end=new Date(start);end.setDate(end.getDate()+7);end.setHours(23,59,59,999);const today=localDate();const reqs=[
  ...
  if(a[7].status==="fulfilled"&&a[7].value){const items=a[7].value.items??[];n.reunioes=items.filter((x:any)=>x.status!=="concluida").reduce((total:number,x:any)=>total+(x.date>=today?1:0)+(x.secondDate>=today?1:0),0)}
  setS(n);
 ...
 const quick=[
  allowed.has("agenda")&&["Agenda","/agenda","▣",null], allowed.has("reunioes")&&["Reuniões","/dashboard/reunioes","👥",s.reunioes],
