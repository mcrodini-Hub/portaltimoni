...
const modules: Array<{
  ...
  {
    name: "Estoque",
    description: "Necessidades, solicitações e respostas do estoque.",
    status: "Disponível",
...
function getTodayRange() {
  ...
  const tomorrowDate = new Date(Date.UTC(Number(value.year), Number(value.month) - 1, Number(value.day) + 1));
...
export default async function DashboardPage() {
  ...
  if (session?.accessToken && session.error !== "RefreshAccessTokenError") {
    try {
      ...
      todayEvents = await listEventsInRange(session.accessToken, { timeMin, timeMax, maxResults: 20 });
    } catch {
      todayEvents = [];
    }
  }
  ...
  return (
    <div className="pb-10">
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-blue-950 to-blue-700 px-6 py-7 text-white shadow-xl shadow-blue-950/10 sm:px-8 sm:py-9">
        <div className="flex flex-col gap-7 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="mb-5 flex items-center gap-3">
              <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-lg font-black text-blue-800 shadow-lg">
                T
              </span>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.22em] text-blue-200">Portal Timoni</p>
                <p className="text-sm text-blue-100/70">Central de gestão da Ciça</p>
              </div>
            </div>
            <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">Olá, Ciça.</h1>
          ...
          <div className="rounded-2xl border border-white/15 bg-white/10 px-5 py-4 backdrop-blur">
            <p className="text-xs font-medium uppercase tracking-wider text-blue-200">Hoje</p>
            <p className="mt-1 capitalize text-sm font-medium text-white">{todayLabel}</p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-6 grid gap-4 lg:grid-cols-[1.4fr_1fr]">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wider text-violet-600">Sua agenda</p>
              <h2 className="mt-1 text-xl font-semibold text-slate-900">
                {todayEvents.length === 0
                  ? "Nenhum compromisso hoje"
                  : `${todayEvents.length} ${todayEvents.length === 1 ? "compromisso" : "compromissos"} hoje`}
              </h2>
            </div>
            <Link
              href="/agenda"
          ...
          <div className="mt-5 space-y-2">
            {todayEvents.slice(0, 3).map((event) => (
              <div key={`${event.calendarKey}-${event.id}`} className="flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3">
                <span className="w-14 text-sm font-semibold text-violet-700">{formatEventTime(event)}</span>
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium text-slate-900">{event.summary}</p>
                  <p className="truncate text-xs text-slate-400">{event.location || event.calendarLabel}</p>
                </div>
              </div>
            ))}
            {todayEvents.length === 0 && (
        ...
        <div className="rounded-3xl border border-blue-100 bg-blue-50 p-6">
          <p className="text-xs font-semibold uppercase tracking-wider text-blue-700">Foco atual</p>
          <h2 className="mt-2 text-xl font-semibold text-slate-900">Estratégia de Compras</h2>
          ...
          <div className="mt-5 rounded-2xl bg-white px-4 py-3 shadow-sm">
            ...
            <p className="mt-1 text-sm font-medium text-slate-800">Pendências, pedidos e conferências</p>
          </div>
        </div>
      </section>
      ...
      <section className="mt-8">
        <div className="flex items-end justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-blue-600">Acesso rápido</p>
            <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900">Seus módulos</h2>
          </div>
          <span className="hidden text-xs text-slate-400 sm:inline">Acesso exclusivo</span>
        </div>
            ...
            const card = (
              <>
                <div className="flex items-start justify-between gap-3">
                  <span className={`flex h-12 w-12 items-center justify-center rounded-2xl ${module.iconClass}`}>
                ...
                <h3 className="mt-5 text-base font-semibold text-slate-900">{module.name}</h3>
