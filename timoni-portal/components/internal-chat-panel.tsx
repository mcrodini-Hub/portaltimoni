...
export default function InternalChatPanel({ open, onClose, onUnreadChange }: { open: boolean; onClose: () => void; onUnreadChange?: (count: number) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end bg-slate-950/10 sm:items-end sm:justify-end sm:p-6" role="dialog" aria-modal="true">
      <button type="button" className="absolute inset-0" onClick={onClose} aria-label="Fechar chat" />
      <section className="relative flex h-[88dvh] w-full max-w-[720px] overflow-hidden rounded-t-[1.75rem] bg-white shadow-[0_24px_70px_rgba(15,23,42,0.22)] sm:h-[560px] sm:rounded-[1.75rem] sm:border sm:border-slate-200">
        <aside className={`${selected ? "hidden sm:flex" : "flex"} w-full flex-col border-r border-slate-200 bg-slate-50/80 sm:w-60`}>
          <div className="border-b border-slate-200 px-4 py-3.5">
            <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-blue-700">Casa Timoni</p>
            <h2 className="mt-0.5 text-base font-semibold text-slate-950">Chat interno</h2>
            <div className="mt-2.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 shadow-sm">
              <input
                value={search}
            ...
            {filteredContacts.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`mb-1 flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2.5 text-left transition ${hasUnread ? "bg-blue-50 ring-1 ring-blue-100 hover:bg-blue-100" : "text-slate-800 hover:bg-white"}`}
                >
                  <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#2296E8] text-xs font-semibold text-white">{item.name.slice(0, 2).toUpperCase()}</span>
        ...
        <div className={`${selected ? "flex" : "hidden sm:flex"} min-w-0 flex-1 flex-col`}>
          <div className="flex min-h-14 items-center gap-2.5 border-b border-slate-200 px-3.5">
            ...
            <button type="button" className="flex h-8 w-8 items-center justify-center rounded-full text-xl leading-none text-slate-500 hover:bg-slate-100" onClick={onClose} aria-label="Fechar chat">×</button>
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto bg-slate-50/50 px-3 py-4 sm:px-4">
            {messages.length === 0 ? <div className="mt-16 text-center text-sm text-slate-500">Envie a primeira mensagem.</div> : <div className="space-y-2.5">{messages.map((message) => { const mine = message.sender_user_id === currentUserId; return <div key={message.id} className="flex"><div className={`max-w-[80%] min-w-[4.75rem] rounded-2xl px-3.5 py-2.5 text-sm font-normal leading-5 shadow-sm ${mine ? "ml-auto bg-[#2296E8] text-white" : "mr-auto border border-slate-200 bg-white text-slate-900"}`}><p className="whitespace-pre-wrap break-words leading-5">{message.body}</p><p className={`mt-1.5 text-right text-xs font-medium ${mine ? "text-white/90" : "text-slate-500"}`}>{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(message.created_at))}</p></div></div>; })}</div>}
          </div>
          <div className="border-t border-slate-200 p-2.5 sm:p-3">{error ? <p className="mb-2 text-xs font-medium text-red-600">{error}</p> : null}<div className="flex items-end gap-2 rounded-2xl border border-slate-300 bg-white p-1.5 shadow-sm focus-within:border-blue-400"><textarea value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); void sendMessage(); } }} rows={1} maxLength={4000} placeholder={contact ? `Mensagem para ${contact.name}` : "Selecione uma conversa"} className="max-h-24 min-h-9 flex-1 resize-none bg-transparent px-2 py-1.5 text-sm text-slate-900 caret-blue-700 outline-none placeholder:text-slate-400" /><button type="button" disabled={!selected || !draft.trim() || sending} onClick={() => void sendMessage()} className="min-h-9 rounded-xl bg-[#2296E8] px-4 text-sm font-semibold text-white disabled:opacity-40">Enviar</button></div></div>
        </div>
      </section>
    </div>
  );
