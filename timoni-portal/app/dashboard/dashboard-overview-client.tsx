...
export default function DashboardOverviewClient({modules,motoristaControle,espacoEquipeControle}:Props){
 ...
 return <div className="grid grid-cols-2 gap-2.5 sm:gap-4 lg:grid-cols-3">{quick.map(([name,href,icon,count])=><Link key={name} href={href} className="relative flex min-h-[112px] min-w-0 flex-col justify-between rounded-xl border border-blue-200 bg-blue-50 p-3 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:min-h-36 sm:rounded-2xl sm:p-5"><div className="flex items-center justify-between"><span className="text-lg text-blue-700 sm:text-2xl">{icon}</span><span className="text-lg text-blue-700">›</span></div><div className="mt-2 sm:mt-4"><p className="truncate text-sm font-semibold text-slate-950 sm:text-base">{name}</p>{count!==null&&count!==undefined&&<p className="mt-1 text-2xl font-bold leading-none text-[#0b1f5e] sm:mt-2 sm:text-3xl">{metric(count)}</p>}</div></Link>)}</div>
}
