...
export default function InternalChatPanel({ open, onClose, onUnreadChange }: { open: boolean; onClose: () => void; onUnreadChange?: (count: number) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end bg-slate-950/20 sm:items-center sm:justify-end sm:p-5" role="dialog" aria-modal="true">
      ...
      <section className="relative flex h-[86vh] w-full overflow-hidden bg-white font-sans shadow-2xl sm:h-[500px] sm:w-[min(90vw,640px)] sm:rounded-2xl sm:border sm:border-slate-200">
        ...
        <div className={`${selected ? "flex" : "hidden sm:flex"} min-w-0 flex-1 flex-col`}>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto bg-slate-50/50 px-3 py-3 sm:px-4">
            {messages.length === 0 ? <div className="mt-16 text-center text-sm text-slate-500">Envie a primeira mensagem.</div> : <div className="space-y-1.5">{messages.map((message) => { const mine = message.sender_user_id === currentUserId; return <div key={message.id} className="flex"><div className={`max-w-[68%] min-w-[4.5rem] rounded-2xl px-3 py-2 text-sm font-normal leading-[1.25rem] shadow-sm ${mine ? "ml-auto bg-[#2296E8] text-white" : "mr-auto border border-slate-200 bg-white text-slate-900"}`}><p className="whitespace-pre-wrap break-words leading-[1.25rem]">{message.body}</p><p className={`mt-0.5 text-right text-[11px] font-normal ${mine ? "text-white/90" : "text-slate-500"}`}>{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(message.created_at))}</p></div></div>; })}</div>}
          </div>
