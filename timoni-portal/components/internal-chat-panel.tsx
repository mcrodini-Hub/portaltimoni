...
type Contact = { name: string; email: string; unread: number };
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end bg-slate-950/20 sm:items-center sm:justify-end sm:p-5" role="dialog" aria-modal="true">
      ...
      <section className="relative flex h-[86vh] w-full max-w-4xl overflow-hidden bg-white shadow-2xl sm:h-[680px] sm:rounded-2xl sm:border sm:border-slate-200">
        <aside className={`${selected ? "hidden sm:flex" : "flex"} w-full flex-col border-r border-slate-200 bg-slate-50 sm:w-72`}>
          <div className="border-b border-slate-200 px-5 py-4"><p className="text-xs font-semibold uppercase tracking-[.16em] text-blue-700">Casa Timoni</p><h2 className="mt-1 text-lg font-semibold text-slate-950">Chat interno</h2></div>
          <div className="flex-1 overflow-y-auto p-2">
            {overview.contacts.map((item) => <button key={item.email} type="button" onClick={() => setSelected(item.email)} className="mb-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-slate-800 transition hover:bg-white"><span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#0F2D8F] text-sm font-semibold text-white">{item.name.slice(0, 2).toUpperCase()}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{item.name}</span><span className="block text-xs text-slate-500">Mensagem direta</span></span>{item.unread > 0 ? <span className="flex min-h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1 text-[11px] font-bold text-white">{item.unread > 99 ? "99+" : item.unread}</span> : null}</button>)}
          </div>
        </aside>
        <div className={`${selected ? "flex" : "hidden sm:flex"} min-w-0 flex-1 flex-col`}>
          <div className="flex min-h-16 items-center gap-3 border-b border-slate-200 px-4"><button type="button" className="px-2 py-2 text-xl text-slate-500 sm:hidden" onClick={() => setSelected(null)}>‹</button><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-slate-950">{contact?.name || "Selecione uma conversa"}</p><p className="text-xs text-slate-500">Chat interno · equipe autorizada</p></div><button type="button" className="rounded-lg px-3 py-2 text-sm text-slate-500 hover:bg-slate-100" onClick={onClose}>Fechar</button></div>
