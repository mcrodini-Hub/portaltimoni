      ...
      <aside className="absolute inset-y-0 left-0 flex w-[84%] max-w-[22rem] flex-col bg-white shadow-2xl">
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-3" aria-label="Menu mobile">
          {items.map((item, index) => {
            const pending = item.updateModule ? updates[item.updateModule] : undefined;
            return <Link key={`${item.label}-${index}`} href={item.targetHref ?? item.href} target={item.external ? "_blank" : undefined} rel={item.external ? "noopener noreferrer" : undefined} onClick={() => void openItem(item)} className={`flex min-h-12 items-center gap-4 rounded-xl px-3 text-[1.05rem] font-medium text-slate-900 ${(item.targetHref ?? item.href) === "/dashboard" ? "bg-blue-50" : ""}`}>
