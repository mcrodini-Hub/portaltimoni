...
export default function InternalChatPanel({ open, onClose, onUnreadChange }: { open: boolean; onClose: () => void; onUnreadChange?: (count: number) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end bg-slate-950/20 sm:items-center sm:justify-end sm:p-5" role="dialog" aria-modal="true">
      ...
      <section className="relative flex h-[86vh] w-full overflow-hidden bg-white font-sans shadow-2xl sm:h-[560px] sm:w-[min(92vw,720px)] sm:rounded-2xl sm:border sm:border-slate-200">
        <aside className={`${selected ? "hidden sm:flex" : "flex"} w-full flex-col border-r border-slate-200 bg-slate-50 sm:w-60`}>
        ...
        <div className={`${selected ? "flex" : "hidden sm:flex"} min-w-0 flex-1 flex-col`}>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto bg-slate-50/50 px-4 py-4 sm:px-5">
            {messages.length === 0 ? <div className="mt-16 text-center text-sm text-slate-500">Envie a primeira mensagem.</div> : <div className="space-y-2">{messages.map((message) => { const mine = message.sender_user_id === currentUserId; return <div key={message.id} className="flex"><div className={`max-w-[78%] min-w-[5rem] rounded-2xl px-3.5 py-2.5 text-sm font-normal leading-5 shadow-sm ${mine ? "ml-auto bg-[#2296E8] text-white" : "mr-auto border border-slate-200 bg-white text-slate-900"}`}><p className="whitespace-pre-wrap break-words leading-5">{message.body}</p><p className={`mt-1.5 text-right text-xs font-medium ${mine ? "text-white/90" : "text-slate-500"}`}>{new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(message.created_at))}</p></div></div>; })}</div>}
          </div>
